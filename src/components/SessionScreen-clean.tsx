...
export function SessionScreen({
  ...
  useEffect(() => {
    if (microphonePermission === "granted" && !sessionInitialized) {
  ...
  }, [microphonePermission, sessionInitialized, isFirstSession, hasIntroduced]);
  ...
  const checkMicrophonePermission = async () => {
    try {
      if (navigator.permissions) {
        ...
        setMicrophonePermission(permission.state as "prompt" | "granted" | "denied");
      }
    } catch (error) {
  ...
  const generateFirstQuestion = async () => {
    ...
    try {
      ...
      const fallbackQuestion = "Tell me about yourself and your experience";
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.name === "NotAllowedError") {
          ...
          setMicrophonePermission("denied");
        } else if (error.name === "NotSupportedError" || error.message.includes("does not support")) {
          setPermissionError("Your browser doesn't support audio recording. Try Chrome, Firefox, or Safari.");
          setMicrophonePermission("denied");
        } else if (error.name === "SecurityError" || error.message.includes("HTTPS")) {
          setPermissionError("Microphone access requires a secure connection (HTTPS). Please access the site over HTTPS.");
          setMicrophonePermission("denied");
        } else if (error.name === "AbortError") {
          setPermissionError("Permission request was interrupted. Please try again.");
          setMicrophonePermission("denied");
        } else {
          setPermissionError(`Microphone setup failed: ${error.message}. Please check your microphone settings.`);
          setMicrophonePermission("denied");
        }
      } else {
        setPermissionError("Microphone access failed. Please check your microphone settings.");
        setMicrophonePermission("denied");
      }
    }
  };
  ...
  const startRecording = async () => {
    try {
      ...
      if (error instanceof Error) {
        if (error.name === "NotAllowedError") {
          setPermissionError("Microphone access was denied. Please allow microphone access.");
          setMicrophonePermission("denied");
        } else if (error.name === "NotFoundError") {
          setPermissionError("No microphone found. Please connect a microphone.");
          setMicrophonePermission("denied");
        } else if (error.name === "SecurityError") {
          setPermissionError("Security error accessing microphone. Please use HTTPS.");
          setMicrophonePermission("denied");
        } else {
          setPermissionError(`Recording failed: ${error.message}. Please check your microphone.`);
        }
      } else {
        setPermissionError("Recording failed - Please check your microphone settings");
      }
    }
  };
  ...
  const stopSession = () => {
    ...
    const validScores = sessionResults.feedbacks.map((f) => f.score);
  ...
  const getStatusMessage = () => {
    ...
    if (microphonePermission === "prompt" && !permissionRequested) {
    ...
    switch (sessionState) {
      ...
      case "recording":
        return "Start speaking your answer now...";
      case "processing":
  ...
  const getTioAvatarState = (): "idle" | "listening" | "speaking" | "thinking" => {
    switch (sessionState) {
      case "introduction":
      case "speaking":
        return "speaking";
      ...
      <div className="w-full max-w-sm space-y-12">
        ...
        <div className="space-y-12">
          <div className="text-center space-y-4">
            <TioAvatar state={getTioAvatarState()} size="lg" />
          ...
          {/* Permission Request */}
          {microphonePermission === "prompt" && !permissionRequested && (
          ...
          {/* Permission Error */}
          {(permissionError || microphonePermission === "denied") && (
            <div className="text-center space-y-4">
          ...
          {questionCount > 0 && sessionState !== "complete" && (
            <div className="text-center space-y-2">
              <div className="flex justify-center space-x-1">
                {Array.from({ length: Math.min(questionCount, 5) }).map((_, i) => (
                  <div
                    key={i}
